...
describe('MakeTree', () => {
  ...
  it('multiple levels of headers', () => {
    expect(MakeTree([h1, h2])).toEqual([treeHeading(h1, [h2])]);

    expect(MakeTree([h1, h2, h1])).toEqual([treeHeading(h1, [h2]), h1]);

    expect(MakeTree([h1, h2, h1, h2])).toEqual([treeHeading(h1, [h2]), treeHeading(h1, [h2])]);
  });

  it('regession test', () => {
    let result = MakeTree([h1, h2, h3, h3, h2, h3, h4]);
    expect(result).toEqual([
      treeHeading(h1, [treeHeading(h2, [h3, h3]), treeHeading(h2, [treeHeading(h3, [h4])])]),
    ]);
  });
});
